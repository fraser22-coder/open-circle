  ...
  try {
    ...
    if (briefSlugs.length > 0) {
      ...
      const { data: picked } = await supabaseAdmin
        ...
        .eq('is_active', true)
        .in('slug', briefSlugs)
      vendorsToNotify = (picked as Vendor[]) ?? []
    } else {
      // No preference → email all active & available vendors in matching categories
