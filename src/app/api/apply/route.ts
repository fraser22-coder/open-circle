...
export async function POST(req: NextRequest) {
  ...
  // ── Build a unique storage folder per application ───────────────────────────
  ...
  for (let i = 0; i < foodPhotos.length; i++) {
    ...
    if (photoError) {
      console.error(`Food photo ${i + 1} upload error:`, photoError)
      // Non-fatal: skip this photo and continue
      continue
    }
